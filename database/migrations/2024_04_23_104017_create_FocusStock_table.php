...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('FocusStock', function (Blueprint $table) {
            ...
            $table->char('userID', 32)->comment('對應用戶ID');
            $table->char('lineAuthCode')->comment('Line推播AuthCode');
            $table->char('stockCode', 32)->comment('股票代碼');
            $table->char('name', 32)->comment('股票名稱');
            $table->string('method', 64)->comment('推播類型');
            $table->integer('toggle')->default(1)->comment('是否啟用推播');
